...
async def login(
    ...
    await create_token(db, form_data.username, access_token)
    ...
    return {"message": "Successfully logged in", "user_email": user.email}
...
async def logout(
    ...
    # Отзываем токен
    await revoke_token(db, current_user, token)
